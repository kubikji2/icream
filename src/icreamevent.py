...
class ICREAMEvent:
    ...
    def parse_component(self, component : Event):
        if component.name == "VEVENT":
            self.name = replace_mult_spaces(component.get('summary'))
            # decoding based on: https://icalendar.readthedocs.io/en/latest/usage.html#more-documentation
            self.start = icream_date_converter(component.decoded('dtstart'))
            self.end = icream_date_converter(component.decoded('dtend'))
            self.organizer = iceam_parse_email(component.get('organizer'))
    ...
    def __str__(self):
        dur_s = icream_duration_formater(self.start, self.end)
